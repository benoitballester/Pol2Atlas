...
df = pd.read_csv("/shared/projects/pol2_chipseq/pol2_interg_default/data_clean/filteredGenic.bed", 
                 sep="\t", header=None)
logQvalCutoff = 5
...
for f in splited.keys():
    if len(splited[f]) > peakCountCutoff:
        splited[f].to_csv(f"/shared/projects/pol2_chipseq/pol2_interg_default/data_clean/peaksIntra/{f}.bed.gz", 

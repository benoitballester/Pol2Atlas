...
def plotReadDistribs(allReads, allCounts, bgCounts, suffix, plotAllCounts=False):
    ...
    if plotAllCounts:
        ...
        plt.show()
        plt.close()
    # Fraction of intergenic counts on RNAP2-bound regions
    ...
    plt.xlim((0,1))
    plt.yticks([], [])
    plt.xlabel("Fraction of intergenic reads on RNAP2-bound regions")
    ...
    """sns.stripplot(data=pctPerSample, jitter=0.33, dodge=True, 
                    edgecolor="black",alpha=1.0, s=2/sf, linewidth=0.1, orient="h") """
    plt.yticks([], [])
    plt.xlabel("Percentage of mapped reads in RNAP2-bound regions")

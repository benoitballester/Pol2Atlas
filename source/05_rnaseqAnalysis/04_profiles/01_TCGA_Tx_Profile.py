...
try:
    ...
    pass
try:
    os.mkdir(paths.outputDir + "rnaseq/profiles/tcga/")
except:
    pass

directory = "/shared/projects/pol2_chipseq/rnaseq_counts_10bp/tcga_10bp/"
...
np.save(paths.tempDir + "pooled_TCGA_10bp.npy", readsPerPol2Bin)
# %%
readsPerPol2Bin = np.load(paths.tempDir + "pooled_TCGA_10bp.npy")
# %%
...
plt.ylabel(f"{hasreads.sum()} Pol II consensuses", fontsize=5)
plt.xlabel(f"Distance to centroid (bp)", fontsize=5)
plt.imshow(downSampledMatrixView, 
...
plt.tight_layout()
plt.savefig(paths.outputDir + "rnaseq/profiles/tcga/profile_1kb_heatmap.pdf", bbox_inches="tight")
plt.show()
# %%
plt.figure(dpi=300)
...
plt.savefig(paths.outputDir + "rnaseq/profiles/tcga/profile_1kb.pdf", bbox_inches="tight")

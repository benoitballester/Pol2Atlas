...
# %%
countTable = pd.DataFrame(counts.T, columns=order)
...
decomp = permutationPA_PCA(countModel.anscombeResiduals[:, hv], mincomp=2)
# Plot PC 1 and 2
plt.figure(dpi=500)
...
allDE[4] = -np.log10(res["padj"][res["padj"].values < 0.05])
# %%
allDE.to_csv(paths.outputDir + "rnaseq/TumorVsNormal2/" + case + "/allDE.bed", sep="\t", header=None, index=None)
enrichedBP = enricher.findEnriched(consensuses[nzCounts][res["padj"] < 0.05], consensuses[nzCounts])
...
# %%
# Plot heatmap and dendrograms (all)
rowOrder, rowLink = matrix_utils.twoStagesHClinkage(decomp, "correlation")
...
rowOrder, rowLink = matrix_utils.twoStagesHClinkage(decomp, "correlation")
colOrder, colLink = matrix_utils.threeStagesHClinkage(countModel.anscombeResiduals.T[hv & nonOutliers], "correlation")
plt.figure(dpi=500)
hierarchy.dendrogram(colLink, p=10, truncate_mode="level", color_threshold=-1)
plt.axis('off')
...
meanNormal = np.mean(anscombe[labels == 0][:, res["padj"] < 0.05], axis=0)
meanTumor = np.mean(anscombe[labels == 1][:, res["padj"] < 0.05], axis=0)
diff = meanTumor - meanNormal
orderRow = np.argsort(diff)
# Only DE
plt.figure(dpi=500)
plot_utils.plotDeHM(anscombe[:, res["padj"] < 0.05][orderCol][:, orderRow], labels[orderCol], np.ones(np.sum(res["padj"] < 0.05)))
...
orderRow = np.argsort(diff)
plt.figure(dpi=500)
plot_utils.plotDeHM(anscombe[orderCol][:, orderRow], labels[orderCol], (res["padj"] < 0.05).astype(float)[orderRow])
plt.savefig(paths.outputDir + "rnaseq/TumorVsNormal2/" + case + "/DE_plot.pdf")
# plt.show()
plt.close()
perCancerDE[case] = np.zeros(allCounts.shape[1])
perCancerDE[case][nzCounts] = np.where(res["padj"] > 0.05, 0.0, np.sign(diff))
upreg = consensuses[nzCounts][(res["padj"] < 0.05) & (diff > 0)]
upreg[4] = -np.log10(res["padj"][(res["padj"] < 0.05) & (diff > 0)])
upreg.to_csv(paths.outputDir + "rnaseq/TumorVsNormal2/" + case + "/DE_upreg.bed", sep="\t", header=None, index=None)
downreg = consensuses[nzCounts][(res["padj"] < 0.05) & (diff < 0)]
downreg[4] = -np.log10(res["padj"][(res["padj"] < 0.05) & (diff < 0)])
...
plot_utils.plotUmap(decomp, colors)
patches = []
...
for train, test in StratifiedKFold(10, shuffle=True, random_state=42).split(decomp, labels):
    ...
    model = catboost.CatBoostClassifier(class_weights=len(labels) / (2 * np.bincount(labels)), 
                                        random_state=42, rsm=np.sqrt(x_train.shape[1])/x_train.shape[1])
